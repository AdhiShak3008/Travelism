...
export async function extractHotels(
  ...
  const focus = priorities.length
    ...
    : "";
  const prompt = corpus
    ? `DESTINATION: ${destination}\n${focus}\n\nFrom the pages, extract 10-18 real, authentic, named hotels/stays in ${destination} across a realistic budget-to-luxury spectrum for this specific destination:\n- REAL BUDGET STAYS: Clean, highly-rated guesthouses, homestays, boutique hostels, and budget rooms (in India/SE Asia: ₹750–₹1,900/night; in Europe/US/Japan: ₹2,400–₹4,800/night). MUST include at least 4-5 verified budget options under 2,000 INR if in India/South Asia/SE Asia.\n- REAL MID-RANGE / BOUTIQUE: Charming heritage inns, 3-star boutique properties, central city hotels\n- REAL 4-STAR & 5-STAR LUXURY: Premium resorts, heritage palaces, luxury landmark hotels\nIMPORTANT: Use genuine hotel names (e.g. "The Resident Covent Garden", "CitizenM Tower of London", "Apex City of Edinburgh", "Zostel Agra", "The Balmoral", not generic templates).\nProvide accurate, market-realistic pricePerNight in INR calibrated for ${destination} (convert local currencies: 1 GBP ≈ 112 INR, 1 EUR ≈ 95 INR, 1 USD ≈ 87 INR).\nInclude: name, location, room, pricePerNight, cleanliness (0-10), bathroomScore (0-10), amenities, policies, hasElevator, overallRating, reviewCount, whyReasons.\nReturn JSON: { "hotels": [ ... ] }\n\nPAGES:\n${corpus}`
    : `DESTINATION: ${destination}\n${focus}\n\nList 10-15 real, authentic, famous and verified hotels/stays in ${destination} across budget, boutique, 4-star, and 5-star luxury tiers with real hotel names (e.g. "Zostel", "The Resident Covent Garden", "CitizenM", "Apex Waterloo Place", "Kimpton Charlotte Square").\n- MUST include 4-5 verified budget-friendly stays starting under ₹1,800/night for domestic/SE Asia destinations, and starting at ₹2,600–₹4,500/night for Western Europe/Nordics/Japan.\nProvide authentic, market-calibrated pricing in INR for ${destination} (1 GBP ≈ 112 INR, 1 EUR ≈ 95 INR, 1 USD ≈ 87 INR), real amenities, clean score (0-10), bathroom score (0-10), policies, and whyReasons.\nReturn JSON: { "hotels": [ ... ] }`;
